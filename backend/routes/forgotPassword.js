...
// OTP store — { email: { otp, expires, verified, resendCount, windowStart } }
const otpStore = {};
    ...
    if (record) {
      ...
      if (timePassed < ONE_HOUR) {
        if (record.resendCount >= MAX_RESENDS) {
          const waitMin = Math.ceil((ONE_HOUR - timePassed) / 60000);
          return res.status(429).json({
            message: `OTP limit reached. Try again in ${waitMin} minute(s).`
          });
        }
        // Same window — count badhao
        otpStore[email].resendCount += 1;
      } else {
        // 1 hour baad — window reset karo
        otpStore[email].resendCount = 1;
        otpStore[email].windowStart = now;
      }
    } else {
      // Pehli baar
      otpStore[email] = { resendCount: 1, windowStart: now };
    }
    // ─────────────────────────────────────────────────────────────

    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    otpStore[email].otp = otp;
    otpStore[email].expires = now + 10 * 60 * 1000;
    otpStore[email].verified = false;
